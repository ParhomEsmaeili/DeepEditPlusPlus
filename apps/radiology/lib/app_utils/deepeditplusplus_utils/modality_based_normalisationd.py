...
the non-CT normalisation is performed using quantiles. 
...
class ImageNormalisation(MapTransform):
    def __init__(self, 
                keys: KeysCollection, 
        ...
        if self.version_param == '2':
            ...
            for key in self.key_iterator(d):
                ...
                if self.modality in self.x_ray_modalities:
                    ...
                    # d[key] = ScaleIntensity(minv=0.0, maxv=1.0)(d[key])#b_min=0.0, b_max=1.0, clip=True)(d[key])
                    # d[key] = NormalizeIntensity()(d[key])
                    d[key] = ScaleIntensityRangePercentiles(lower=2.5, upper=97.5, b_min=0, b_max=1)
