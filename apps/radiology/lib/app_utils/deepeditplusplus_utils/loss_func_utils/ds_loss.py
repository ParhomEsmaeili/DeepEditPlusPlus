...
class DeepSupervisionLoss(_Loss):
    ...
    def forward(self, input: Union[None, torch.Tensor, list[torch.Tensor]], target: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(input, torch.Tensor) and len(input.shape) != len(target.shape): #In this scenario it is presented in BFNHWD format where F = number of feature maps.
            ...
            #This next operation performs a batchwise loss computation for each feature map, and then sums across the feature maps. 

            #This is the same as performing a sum across feature maps for each batch sample prior to the loss reduction as long as the number of voxels are 
            #consistent across the batch and feature maps (might be a problem for masked losses?).

            for l in range(len(unbound_input)):
                loss += weights[l] * self.get_loss(unbound_input[l].float(), target)
            return loss 
        if input is None:
